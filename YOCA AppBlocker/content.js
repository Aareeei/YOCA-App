...
window.addEventListener("message", function (event) {
    if (event.data && event.data.type === "FOCUS_MODE") {
      if (typeof chrome !== "undefined" && window.chrome.storage?.local) {
        window.chrome.storage.local.set({ focusMode: event.data.enabled }, () => {
          console.log("Focus Mode set in extension:", event.data.enabled);
        });
      } else {
